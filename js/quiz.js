...
export class Quiz {
      ...
      for (const word of selected) {
        ...
        if (this.mode === 'mcq') {
          ...
          this.questions.push({
            ...
            promptSub: direction === 'b2e' ? toPronunciation(word.burmese_word, { tones: false }) : '',
            options
          });
        } else {
          ...
          this.questions.push({
            ...
            promptSub: toPronunciation(word.burmese_word, { tones: false }),
  ...
  initMatchState(pairs) {
    const leftItems = pairs.map(w => ({
      id: w.id, burmese: w.burmese_word, deva: toPronunciation(w.burmese_word, { tones: false })
    })).sort(() => Math.random() - 0.5);
          ...
          <div style="display:flex; flex-direction:column; gap:6px; margin-bottom:20px;">
            ${wrong.map(a => `
              <div style="display:flex; align-items:center; gap:10px; padding:10px 14px; border-radius:12px;
                ...
                <div style="font-size:11px; color:var(--yellow);">${toPronunciation(a.word.burmese_word, { tones: false })}</div>
